export default function Input({ label, error, hint, id, className = '', required, ...props }) {
  ...
  return (
    <div className={className}>
      ...
      <input
        id={inputId}
        className={`w-full bg-surface border rounded-sm px-3.5 py-2.5 text-[14.5px] text-charcoal placeholder:text-muted/70 transition-colors focus:outline-none focus:border-forest ${
      ...
      {!error && hint && (
        <p id={`${inputId}-hint`} className="mt-1.5 text-[12.5px] text-muted">
          {hint}
        </p>
      )}
    </div>
