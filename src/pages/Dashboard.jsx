  ...
  return (
    <Layout title="Overview" description="Your clinical workspace at a glance">
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-10">
        <div>
          <p className="text-[13px] text-muted uppercase tracking-wide mb-2">Good morning, {doctor?.name?.split(' ')[1] || 'Doctor'}</p>
          <h2 className="font-serif text-3xl text-charcoal max-w-lg leading-snug">
      ...
      {attention.length > 0 && (
        <div className="mb-10">
          <h3 className="text-[13px] uppercase tracking-wide text-muted mb-3">Cases requiring attention</h3>
          <div className="flex flex-col gap-2">
            {attention.map((r) => (
              <button
                key={r.id}
                onClick={() => navigate(`/reports/${r.id}`)}
                className="w-full flex items-center gap-3 border border-warning/30 bg-warning/[0.06] rounded-sm px-4 py-3 text-left hover:bg-warning/10 transition-colors"
              >
                <AlertTriangle size={16} className="text-warning shrink-0" />
                <span className="text-[14px] text-charcoal flex-1">
                  <span className="font-medium">{r.patientId}</span> — {r.examination} needs clinician review
                </span>
                <span className="text-[12.5px] text-muted shrink-0">{r.date}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <div>
        <h3 className="text-[13px] uppercase tracking-wide text-muted mb-3">Recent cases</h3>
